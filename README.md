...
5. Go to https://github.com/makehumancommunity/makehuman-assets, download as zip, extract it, copy `base` directory (the entire directory, not its contents) into `makehuman/data` and contents of that directory data directory into `documents/makehuman/v1py3/data` (overwrite duplicates).
6. `cd makehuman` (the nested directory, not the root)
...
An alternative way to install dependencies is using **pip**. However, it is best practice to set up an [virtual environment](https://docs.python.org/3/library/venv.html)
...
figure out how to run [**pip**](https://pip.pypa.io/en/stable/) (it should have been installed by python automatically):
